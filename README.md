...
> 本 skill 只做「流程编排与文档整理」。真正的入库动作依赖 `ima-mcp` 连接器的工具（`create_media` / `add_knowledge` / `get_knowledge_list` / `get_addable_knowledge_base_list`），图片下载与 Base64 内嵌由本 skill 附带的脚本完成。**工具清单以 ima-mcp 服务端实际下发的为准，参数不要凭记忆猜。**
...
- **原文保真**：深度结构化整理但不改原意；数据、表格原样照录；代码 / 提示词 / 案例全文逐字保留；抓不全或不确定处用【存疑】标注。
...
1. **深度总结，且原文内容与意思不变。** 文首加内容概要；按原文逻辑分节重组；不得删减、改写失真、脑补。
2. **数据、图片、表格原样保存，不用外部引用或链接。** 表格用原生 `| 列 | 列 |`；图片 Base64 内嵌；数字照录不四舍五入。
3. **代码、AI 提示词、实例、案例全文保留。** 用围栏代码块逐字保留，不做删节、不转述。
...
| 图片规格统一 | Step 1d | `scripts/normalize_images.py`（转 WebP / 限 1600px） |
| 整理为 Markdown | Step 2 | `scripts/build_md.py` → `body.md` |
| 图片内嵌 | Step 2c | `scripts/embed_base64.py` → 自包含 md |
...
| `scripts/build_md.py` | 中间稿 + `mapping.json` 组装正文 md |
...
- 当前版本：**v2.6.0**（详见 [`SKILL.md`](./SKILL.md) 顶部 `version` 字段）。
